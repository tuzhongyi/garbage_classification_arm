...
import './ai-event-index.less'
import { AIEventIndexMessage } from './ai-event-index.message'

export namespace AIEventIndex {
  export class HtmlController {
    ...
    index = LocalStorageService.navigation.ai.event.get()
      ...
      if (this.element.items) {
        ...
        if (index >= 0) {
          ...
          if (this.element.iframe) {
            ...
            this.index = index
            LocalStorageService.navigation.ai.event.save(this.index)
          }
        }
      }
    }
    ...
    private factory(index: number): string {
      switch (index) {
        case 0:
          return '../ai-event-deployment/ai-event-deployment.html'
        case 1:
          return '../ai-event-rule/ai-event-rule.html'
        default:
          return ''
      }
    }
