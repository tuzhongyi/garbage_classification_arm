...
import './network-frp-index.less'
import { NetworkFrpIndexMessage } from './network-frp-index.message'

export namespace NetworkFrpIndex {
  export class HtmlController {
    ...
    index = LocalStorageService.navigation.network.frp.get()

    element = {
      items: document.getElementsByClassName('menu-item'),
      iframe: document.querySelector('#iframe') as HTMLIFrameElement,
    }
    private message = new NetworkFrpIndexMessage(this.element.iframe)
      ...
      if (this.element.items) {
        ...
        if (index >= 0) {
          ...
          if (this.element.iframe) {
            ...
            this.index = index
            LocalStorageService.navigation.network.frp.save(this.index)
          }
        }
      }
    }
    ...
    private factory(index: number): string {
      switch (index) {
        case 0:
          return '../network-frp-list/network-frp-list.html'
        default:
          return ''
      }
    }
