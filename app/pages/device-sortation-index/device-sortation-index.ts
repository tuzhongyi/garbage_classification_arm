...
import './device-sortation-index.less'
import { DeviceSortationIndexMessage } from './device-sortation-index.message'

export namespace DeviceSortationIndex {
  export class HtmlController {
    constructor() {
      this.regist()
      this.init()
    }
    index = LocalStorageService.navigation.device.sortation.get()
    ...
    private message = new DeviceSortationIndexMessage(this.element.iframe)
    standby?: string
      ...
      if (this.element.items) {
        ...
        if (index >= 0) {
          ...
          if (this.element.iframe) {
            ...
            LocalStorageService.navigation.device.sortation.save(this.index)
          }
        }
      }
    }
    ...
    private factory(index: number, id?: string): string {
      ...
      switch (index) {
        case 0:
          return '../device-sortation-list/device-sortation-list.html'
        case 1:
          return `../device-sortation-info/device-sortation-info.html${params}`
        case 2:
          return `../device-sortation-calibration/device-sortation-calibration.html${params}`
        case 3:
          return `../device-sortation-play/device-sortation-play.html${params}`
        case 4:
          return `../device-sortation-config/device-sortation-config.html${params}`
        case 5:
          return `../device-sortation-log/device-sortation-log.html${params}`
        default:
          return ''
      }
    }
