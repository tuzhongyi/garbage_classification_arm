...
export interface DeviceDropPortDetailsChartHtmlEvent {
...
export class DeviceDropPortDetailsChartHtmlController {
  event = new EventEmitter<DeviceDropPortDetailsChartHtmlEvent>()
  ...
  private regist() {
    this.element.img.addEventListener('load', (x) => {
      let img = x.currentTarget as HTMLImageElement
      wait(
        () => {
