...
import './event-record-index.less'
import { EventRecordIndexMessage } from './event-record-index.message'

export namespace EventRecordIndex {
  export class HtmlController {
    ...
    index = LocalStorageService.navigation.event.record.get()

    element = {
      items: document.getElementsByClassName('menu-item'),
      iframe: document.querySelector('#iframe') as HTMLIFrameElement,
    }
    private message = new EventRecordIndexMessage(this.element.iframe)
      ...
      if (this.element.items) {
        ...
        if (index >= 0) {
          ...
          if (this.element.iframe) {
            ...
            this.index = index
            LocalStorageService.navigation.event.record.save(this.index)
          }
        }
      }
    }
    ...
    private factory(index: number): string {
      switch (index) {
        case 0:
          return '../event-record-list/event-record-list.html'
